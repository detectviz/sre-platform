...
- [9. 告警規則 (Alert Rules)](#9-告警規則-alert-rules)
- [10. 靜音規則 (Incident Silences)](#10-靜音規則-incidents-silences)
- [11. 容量規劃 (Capacity Planning)](#11-容量規劃頁面-capacity-planning-page)
- [12. 腳本庫 (Automation Scripts)](#12-腳本庫-scripts)
- [13. 排程管理 (Automation Schedules)](#13-排程管理-schedules)
- [14. 執行日誌 (Automation Executions)](#14-執行日誌-executions)
- [15. 團隊管理 (Administration Teams)](#15-團隊管理-team-management)
- [16. 角色管理 (Administration Roles)](#16-角色管理-role-management)
- [17. 通知管道 (Administration Channels)](#17-通知管道-notification-channels)
- [18. 系統設定 (Administration Settings)](#18-系統設定-system-settings)
- [19. 平台診斷 (Administration Diagnostics)](#19-平台診斷-platform-diagnostics)
- [20. 審計日誌 (Administration Audit)](#20-審計日誌-audit-logs)
- [21. 個人資料 (Profile)](#21-個人資料與設定-profile--settings)
...
| 英文術語 | 標準中文譯名 | 備註 |
|---------|-------------|------|
| Alert | 告警 | 系統根據預設規則自動觸發的監控信號 |
| Incident | 事件 | 由告警觸發或手動創建，需要人工介入處理的工單 |
| 資源 | 資源 | 監控對象，如伺服器、服務 |
| group | 群組 | 資源邏輯分組 |
| 腳本 | 腳本 | 自動化腳本 |
| 排程 | 排程 | 任務排程計劃 |
...
| personnel | 使用者 | 同 使用者 |
| role | 角色 | 用戶角色權限 |
| permission | 權限 | 具體操作權限 |
| diagnostic | 診斷 | 系統診斷功能 |
| check | 檢查 | 健康檢查 |

#### **術語使用原則**
1. **優先使用中文術語**：在中文文檔中優先使用標準中文譯名
2. **技術術語保留英文**：API 路徑、代碼相關術語保留英文
3. **一致性優先**：在同一文檔內避免混用不同譯名
4. **上下文適配**：根據具體上下文選擇最合適的術語

#### **禁止使用的混用組合**
- ❌ 腳本/腳本 混用 → ✅ 統一使用「腳本」
- ❌ 排程/排程/排程 混用 → ✅ 統一使用「排程」
- ❌ 執行/執行/執行/執行 混用 → ✅ 統一使用「執行」
- ❌ 通知/通知/管道/channel 概念混淆 → ✅ 明確區分使用
- ❌ 使用者/使用者/使用者/使用者 混用 → ✅ 統一使用「使用者」
...
## **11. 容量規劃頁面 (Capacity Planning Page)**
...
* **API 端點**:
  ...
  * `POST /api/v1/analyzing/capacity/forecast`
