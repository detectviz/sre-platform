...
| check | 檢查 | 健康檢查 |
| Maintenance Window | 維護窗口 (靜音規則) | 指計畫性的維護或已知問題期間，用以自動抑制事件通知的規則。等同於「靜音規則」。 |
...
### **9.1. 功能目標**
提供一個界面，用以查看由監控系統直接觸發的、未經整合的原始告警 (Alerts)。此功能主要用於底層問題的追蹤與診斷，與處理需要人工介入的「事件 (Incidents)」有所區別。

### **9.2. 功能規格**
- **列表與篩選**: 提供基於時間、嚴重性、狀態和資源的篩選功能。
- **唯讀性質**: 此頁面主要為唯讀，用於分析，不提供對告警的直接操作（如確認、解決），這些操作應在「事件列表」中進行。
