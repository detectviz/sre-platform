...
#### D1. 全域組件與認證基礎
- [x] **登入頁面** ([images/00-頁面-登入.png](images/00-頁面-登入.png))
- [x] **使用者選單** ([images/01-彈窗-使用者選單.png](images/01-彈窗-使用者選單.png))
- [x] **通知中心** ([images/02-彈窗-通知中心.png](images/02-彈窗-通知中心.png))
- [x] **身份認證整合** - Keycloak 整合實現
📋 **實現摘要**：重構應用程式以使用 `react-router-dom` 進行路由管理。建立了 `AuthContext` 來處理全域認證狀態，並實作了登入頁面 (`LoginPage`)、私有路由 (`PrivateRoute`) 和與上下文連動的使用者選單 (`UserMenu`)。通知中心 (`NotificationCenter`) 現在能從模擬 API 獲取資料並提供互動功能。此基礎為後續整合 Keycloak 等真實身份提供商奠定了基礎。
...
- [x] **事件規則編輯彈窗**
- [x] **建立靜音規則** ([images/18-彈窗-事件規則-建立靜音.png](images/18-彈窗-事件規則-建立靜音.png))

#### B3. 靜音規則管理
- [x] **靜音規則頁面** ([images/12-頁面-靜音規則.png](images/12-頁面-靜音規則.png))
- [x] **靜音規則編輯彈窗**
📋 **實現摘要**：完成了從活躍事件快速建立一次性靜音規則的功能。這包括：新增 `/api/v1/silence-rules` 端點至 `openapi.yaml` 並在 `mock-server` 中實現；建立一個新的、可重用的 `CreateSilenceModal` React 元件，該元件具有讀取事件詳情、顯示匹配標籤和提交靜音設定（時長與原因）的功能；建立 `useCreateSilence` hook 來處理 API 互動；並將新模態框整合至 `IncidentsPage` 的事件操作中，取代了舊有的簡易實現。

### 🤖 AI Agent C: 資源管理與儀表板 ⭐⭐⭐
**負責領域**: 資源管理、儀表板系統

#### C1. 資源管理模組
- [x] **資源列表頁面** ([images/30-頁面-資源列表.png](images/30-頁面-資源列表.png))
- [x] **資源群組管理** ([images/31-頁面-資源群組.png](images/31-頁面-資源群組.png))
- [x] **拓撲視圖** ([images/32-頁面-拓撲視圖.png](images/32-頁面-拓撲視圖.png))
📋 **實現摘要**：為解決大型組件難以維護的問題，將 `ResourcesPage.tsx` 重構成一個容器組件和多個獨立的頁籤組件 (`ResourceOverviewTab`, `ResourceGroupsTab`, `ResourceTopologyTab`)。資源列表頁面已符合設計，並提供了完整的資源群組 CRUD 功能（透過彈出式表單）。拓撲視圖使用 ECharts 實現，支持深色主題和互動式操作，並優化了其版面配置以提高可讀性。
...
#### C3. 自動化中心
- [x] **容量規劃** ([images/50-頁面-容量規劃.png](images/50-頁面-容量規劃.png))
...
📋 **實現摘要**：建立了全新的容量規劃頁面 `CapacityPlanningPage`，其中包括一個用於模擬資料獲取的 `useCapacityPlanning` hook 和一個可重用的 `ForecastChart` ECharts 圖表組件。此頁面能夠根據所選服務，視覺化展示 CPU、記憶體和磁碟的歷史使用數據與未來預測，並提供具體的擴容建議。已將該頁面整合到主應用的路由和導航選單中。

### 🤖 AI Agent E: 通知與平台管理 ⭐⭐⭐
**負責領域**: 通知管理、平台管理、個人資料

#### E1. 通知管理
- [x] **通知策略** ([images/70-頁面-通知策略.png](images/70-頁面-通知策略.png))
- [x] **通知管道** ([images/71-頁面-通知管道.png](images/71-頁面-通知管道.png))
- [x] **通知歷史** ([images/72-頁面-通知歷史.png](images/72-頁面-通知歷史.png))

#### E2. 平台管理
- [x] **標籤管理頁面** ([images/80-頁面-標籤管理.png](images/80-頁面-標籤管理.png))
- [x] **郵件設定** ([images/81-頁面-郵件設定.png](images/81-頁面-郵件設定.png))
- [x] **身份驗證** ([images/82-頁面-身份驗證.png](images/82-頁面-身份驗證.png))

#### E3. 個人資料模組
- [x] **個人資訊** ([images/90-頁面-個人資料-個人資訊.png](images/90-頁面-個人資料-個人資訊.png))
- [x] **偏好設定** ([images/91-頁面-個人資料-偏好設定.png](images/91-頁面-個人資料-偏好設定.png))
- [x] **密碼安全** ([images/92-頁面-個人資料-密碼安全.png](images/92-頁面-個人資料-密碼安全.png))
📋 **實現摘要**：完成了通知管理、平台管理與個人資料三大模組的前端開發。
- **通知管理**: 建立了 `useNotificationPolicies` hook，並將 `NotificationManagementPage` 中所有頁籤（策略、管道、歷史）與後端 API（模擬）完全整合，取代了原有的靜態資料。
- **平台管理**: 根據設計稿，在 `PlatformSettingsPage` 中實現了「標籤管理」、「郵件設定」和「身份驗證」的完整表單介面。為標籤管理建立了 `useTags` hook，並在 `db.json` 中擴充了模擬資料。
- **個人資料**: 完善了 `ProfilePage` 的「偏好設定」功能，建立了 `useUserProfile` hook，使主題、時區等設定能夠從後端（模擬）載入並儲存。
...
#### D2. 身份與存取管理
- [x] **人員管理** ([images/60-頁面-人員管理.png](images/60-頁面-人員管理.png))
- [x] **團隊管理** ([images/61-頁面-團隊管理.png](images/61-頁面-團隊管理.png))
- [x] **角色管理** ([images/62-頁面-角色管理.png](images/62-頁面-角色管理.png))
- [x] **審計日誌** ([images/63-頁面-審計日誌.png](images/63-頁面-審計日誌.png))
- [x] **設定頁面** ([images/64-頁面-設定.png](images/64-頁面-設定.png))
📋 **實現摘要**：建立了統一的身份與存取管理頁面 (`UserPermissionsPage`)，透過頁籤 (Tabs) 整合了人員和團隊管理。所有列表均從 API 動態載入資料，並提供了完整的 CRUD 功能，包括邀請人員、編輯權限、建立團隊等，皆透過 Modal 彈出視窗完成。同時，實作了獨立的角色管理 (`RoleManagementPage`) 和審計日誌頁面 (`AuditLogPage`)，並建立了設定頁面 (`SettingsPage`) 作為所有管理功能的導覽中樞。
