...
* **設計重點**:
  ...
  * **權限管理 (Permission Management)**: 為每個人員或角色提供詳細的權限配置界面，可通過勾選框或下拉選單進行設定。
  * **邀請與添加 (Invite & Add)**: 明確的「邀請人員」或「添加團隊」按鈕，引導人員進行相關操作。
  * **編輯與刪除 (Edit & Delete)**: 提供行級操作按鈕（如編輯圖標、刪除圖標），方便管理單個人員或團隊。
  * **搜索與篩選 (Search & Filter)**: 針對人員名、角色、團隊等屬性提供搜索和篩選功能。
* **頁面說明**:  
  * **[使用者管理頁面]**: 畫面頂部是「使用者管理」和「團隊管理」的頁籤，當前選中的是「使用者管理」。頁籤下方是操作區，包含搜尋框和藍色的「**邀請人員**」按鈕。主體部分是一個 Ant Design Table，列出了人員 ID、姓名、郵箱、角色（使用不同顏色的 Tag 標示）、所屬團隊等資訊。
  * **[邀請/編輯使用者彈窗]**: 點擊「邀請人員」後，會彈出一個 Modal，表單僅包含**姓名、郵箱**等邀請所需欄位。點擊現有使用者的「編輯」按鈕，彈出的 Modal 中，姓名、郵箱等欄位為**唯讀**，主要操作集中在修改其**角色**和**所屬團隊**。
...
### **16.4. 開發規格**
- **核心元件**: `OrganizationPage.tsx`, `UserTable.tsx`, `TeamTable.tsx`, `InviteUserModal.tsx`, `EditUserPermissionsModal.tsx`, `TeamFormModal.tsx`
- **API 端點**:
    - `GET /api/v1/users`: 獲取人員列表。
    - `POST /api/v1/invitations`: 邀請新人員。
    - `GET /api/v1/users/{userId}`: 獲取特定人員的詳情。
    - `PUT /api/v1/users/{userId}`: **僅更新**人員的平台角色與團隊。
    - `PUT /api/v1/users/{userId}/internal-status`: 更新人員的平台內部啟用狀態。
    - `DELETE /api/v1/users/{userId}`: **軟刪除**人員。
    - `GET /api/v1/teams`: 獲取團隊列表。
    - `POST /api/v1/teams`: 創建新團隊。
    - `GET /api/v1/teams/{teamId}`: 獲取特定團隊的詳情。
    - `PUT /api/v1/teams/{teamId}`: 更新團隊資訊。
    - `DELETE /api/v1/teams/{teamId}`: **軟刪除**團隊。
    - `POST /api/v1/teams/{teamId}/members`: 管理團隊成員（新增或移除成員）。
...
- **API 端點**:
    - `GET /api/v1/admin/roles`: 獲取角色列表（供 UI 顯示角色名稱與描述，所有登入人員可讀取）。
...
- **隱藏操作按鈕**：當人員無權限時，不顯示「新增」、「編輯」、「刪除」等按鈕
...
* **視覺設計描述**:
    ...
    - **設定表單**:
        ...
        - **郵件伺服器設定卡片**:
            - 包含 SMTP 伺服器地址 (`Input`)、埠號 (`InputNumber`)、人員名 (`Input`)、密碼 (`Input.Password`) 等欄位。
    - **操作區域**:
...
### **22.1. 功能目標**
提供使用者個人化設定的入口，並與身份提供商 (Keycloak) 的職責明確劃分。

### **22.2. 功能規格**
- **頁面佈局**: 使用**頁籤 (`Tabs`)** 佈局，包含「個人資訊」、「密碼安全」、「通知偏好」等設定。
- **個人資訊 (唯讀)**:
    - **功能**: **唯讀地**展示從 Keycloak 同步的使用者核心資訊（如姓名、電子郵件）。
    - **操作**: 不提供任何表單或儲存按鈕。頁面中應包含一個**明確的超連結**，文字為「**前往 Keycloak 管理個人資料**」，引導使用者到外部身份提供商的帳號管理中心進行修改。
- **密碼安全 (僅引導)**:
    - **功能**: 不提供任何修改密碼的表單。
    - **操作**: 頁面中應包含一個**明確的按鈕或超連結**，文字為「**前往 Keycloak 修改密碼或設定 2FA**」，將使用者重定向至 Keycloak 的安全設定頁面。
- **通知偏好 (平台功能)**:
    - **功能**: 允許使用者管理**平台內部**的通知接收偏好，此功能與核心身份無關。
...
* **頁面說明**:  
  ...
  * **[個人資料頁籤]**: 內部再次使用 Tabs 將「個人資訊」、「密碼安全」和「通知設定」三個部分清晰地劃分開來。  
    * 在「個人資訊」頁籤中，姓名和郵件地址為純文字，下方有一個前往 Keycloak 的連結。
    * 在「密碼安全」頁籤中，沒有輸入框，只有一個前往 Keycloak 的按鈕。
    * 在「通知設定」頁籤中，每個聯絡方式（Email, SMS）後面都有一個狀態指示器（例如「已驗證」的綠色標籤）和獨立的操作按鈕（「傳送驗證」、「測試」），介面清晰地傳達了每個聯絡渠道的狀態和可用操作。  

### **22.4. 開發規格**
- **核心元件**: `ProfilePage.tsx`, `ProfileInfoTab.tsx`, `SecurityRedirectTab.tsx`, `NotificationPreferencesTab.tsx`
- **API 端點**:
    - `GET /api/v1/users/me`: 獲取當前登入使用者的資訊 (從 JWT 解析)。
    - `GET /api/v1/users/me/preferences`: 獲取通知偏好設定。
    - `PUT /api/v1/users/me/preferences`: 更新通知偏好設定。
- **驗收條件**:
    - [ ] 「個人資訊」頁籤中的使用者資料為唯讀。
    - [ ] 「個人資訊」和「密碼安全」頁籤中包含指向 Keycloak 的正確連結。
    - [ ] 使用者能成功設定並儲存**通知偏好**。
    - [ ] 介面上沒有任何修改個人資料或密碼的表單。